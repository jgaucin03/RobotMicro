...
#
# Robot-real planner (REAL-FRAME EXPORT VERSION):
# - Identical planning pipeline to planner_numerical.py:
#     * Multi-stage manual waypoint lists (Stage 1..5)
#     * Optional IK-refine stage ends to target sites
#     * Unwrap across ALL stage boundaries (avoid pi discontinuities)
#     * Retiming with velocity limits at fixed CTRL_DT
#     * Piecewise gripper schedule (open/close/settle/tuck)
#     * Outputs CSV with 8 columns: 6 arm joints + gripper_ctrl + t
#
# NEW: UR3 REAL joint frame export (offsets vs MuJoCo model):
#   real = mjc + offsets, where:
#     shoulder_lift offset = -pi/2
#     wrist_1       offset = -pi/2
#   others offset = 0
#
# This script writes BOTH:
#   1) MuJoCo frame outputs (for easy sim playback / pick_cup.py)
#   2) Real-robot frame outputs (for physical execution)
#
# Output files:
#   - motion/path_latest_with_gripper.csv                 (MJC, convenience, always overwritten)
#   - motion/plan_stage12345_robotreal_<ts>.csv           (MJC, archived)
#   - motion/path_real_mjcframe_latest.csv                (MJC, always overwritten)
#   - motion/path_real_mjcframe_<ts>.csv                  (MJC, archived)
#   - motion/path_real_latest.csv                         (REAL, always overwritten)
#   - motion/path_real_<ts>.csv                           (REAL, archived)
...
# Convenience / compatibility outputs
OUT_LATEST_SIM_MJC = os.path.join(CURRENT_DIR, "path_latest_with_gripper.csv")

# Real-vs-MJC named outputs
...
# Leave later-stage IK off by default (hand-authored microwave motion)
PLAN_STAGE_3_SITE = None  # e.g. "cup_target_site"
PLAN_STAGE_4_SITE = None
PLAN_STAGE_5_SITE = None

# -------------------------
# Manual waypoints (MuJoCo frame)
# -------------------------
...
MANUAL_WAYPOINTS_STAGE_3 = [
    ...
    # "shoulder_lift: -1.85 -> -1.79" intermediate
...
MANUAL_WAYPOINTS_STAGE_4 = [
    # open the gripper (release cup), then small move out
    np.array([-1.26, -1.85, 0.0628, -0.88, -1.57, -1.82], dtype=float),
    np.array([-1.26, -1.38, 0.0628, -0.88, -1.57, -1.82], dtype=float),
]

MANUAL_WAYPOINTS_STAGE_5 = [
    # tuck / retreat (often with gripper closed)
...
def main():
    ...
    # Full arm + grip (MuJoCo frame joints)
    arm_full_mjc = np.vstack([
        arm12,
        arm_close,
        arm_settle_closed,
        arm3,
    ...
    assert arm_full_mjc.shape[0] == grip_full.shape[0], "arm/grip length mismatch"

    # Convert arm joints to REAL frame (joint offsets)
    print("\n=== REAL JOINT FRAME CONVERSION (APPLIED TO FULL TRAJECTORY) ===")
    ...
    arm_full_real = apply_real_offsets(arm_full_mjc, REAL_JOINT_OFFSETS)

    print("\nSanity check (real - mjc) on first arm row:")
    print(f"  arm_full_real[0] - arm_full_mjc[0] = {arm_full_real[0] - arm_full_mjc[0]}")
    print("Sanity check (real - mjc) on last arm row:")
    print(f"  arm_full_real[-1] - arm_full_mjc[-1] = {arm_full_real[-1] - arm_full_mjc[-1]}")

    # Build both plans: [q1..q6, gripper_ctrl]
    header = ARM_JOINT_NAMES + ["gripper_ctrl"]

    plan_mjc = np.hstack([arm_full_mjc, grip_full[:, None]])
    plan_real = np.hstack([arm_full_real, grip_full[:, None]])
    ...
    print("\n=== COMPILED PLAN SUMMARY (MJC FRAME: ARM+GRIPPER_CTRL+t) ===")
    ...
    print("\n=== COMPILED PLAN SUMMARY (REAL FRAME: ARM+GRIPPER_CTRL+t) ===")
    ...
    # Archived filenames
    out_csv_mjc_stage = os.path.join(CURRENT_DIR, f"plan_stage12345_robotreal_{out_ts}.csv")  # MJC archived (compat naming)
    out_csv_mjc_named = os.path.join(CURRENT_DIR, f"path_real_mjcframe_{out_ts}.csv")
    out_csv_real      = os.path.join(CURRENT_DIR, f"path_real_{out_ts}.csv")

    # Write MJC outputs
    write_plan_csv(out_csv_mjc_stage, plan_mjc, header)
    write_plan_csv(OUT_LATEST_SIM_MJC, plan_mjc, header)
    write_plan_csv(out_csv_mjc_named, plan_mjc, header)
    write_plan_csv(OUT_LATEST_MJC, plan_mjc, header)

    # Write REAL outputs
    write_plan_csv(out_csv_real, plan_real, header)
    write_plan_csv(OUT_LATEST_REAL, plan_real, header)

    print("\n[RESULT] Wrote MuJoCo-frame (sim) CSVs:")
    print(f"  - {out_csv_mjc_stage}")
    print(f"  - {out_csv_mjc_named}")
    print(f"  - {OUT_LATEST_MJC}")
    print(f"  - {OUT_LATEST_SIM_MJC}  (compat: pick_cup.py default)")
    print("\n[RESULT] Wrote REAL-frame (physical) CSVs:")
    print(f"  - {out_csv_real}")
    print(f"  - {OUT_LATEST_REAL}")
    print(f"\n[RESULT] CTRL_HZ={CTRL_HZ:.1f} => approx plan duration = {plan_mjc.shape[0] * CTRL_DT:.2f} s")
